...
class DocumentController extends Controller
{
    ...
    // Rewrite subsection content using OpenAI
    public function rewriteSubsectionContent(Request $request, Document $document, DocumentSubsection $subsection)
    {
        if ($subsection->document_id !== $document->id) {
            return response()->json(['message' => 'Subsection not found'], 404);
        }
        ...
        $currentContent = $subsection->content_html;
        ...
        try {
            ...
            $prompt = "Rewrite the following text content from an energy audit report to be professional, clear, and well-structured. Keep it simple and direct. Maintain all key information and technical details. Use plain text only, no special formatting characters:\n\n" . $plainText;
            ...
            if (!isset($responseData['choices'][0]['message']['content'])) {
                ...
                return response()->json(['error' => 'Failed to generate rewritten content.'], 500);
            }
            ...
            return response()->json(['error' => 'Failed to rewrite content: ' . $e->getMessage()], 500);
        } catch (\Exception $e) {
            \Illuminate\Support\Facades\Log::error('Error rewriting subsection content: ' . $e->getMessage());
            return response()->json(['error' => 'An error occurred while rewriting the content.'], 500);
        }
    }
