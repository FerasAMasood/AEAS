...
<body>
@php
    $formatPdfCell = static function ($value): string {
        if ($value === null) {
            return '';
        }
        if (is_string($value)) {
...
@endphp
@php
    $tocPageNumbers = is_array($tocPageNumbers ?? null) ? $tocPageNumbers : [];
@endphp
    ...
    @php
        $propertyName = $report->property->property_name ?? '';
        $tocSections = [
            ['key' => 'abbreviations', 'label' => 'Abbreviations'],
            ['key' => 'tariff_values', 'label' => 'Tariff Values'],
            ['key' => 'executive_summary', 'label' => 'Executive Summary'],
            ['key' => 'general_description', 'label' => 'General Description'],
            ['key' => 'introduction', 'label' => 'Introduction'],
            ['key' => 'property_site', 'label' => $propertyName !== '' ? ($propertyName.' site') : 'Property site'],
            ['key' => 'property_devices', 'label' => 'Property Devices'],
            ['key' => 'electricity_balance', 'label' => 'Electricity Balance'],
            ['key' => 'pie_chart', 'label' => 'Power Consumption Pie Chart'],
            ['key' => 'recommendations_overview', 'label' => 'Recommendations (overview)'],
            ['key' => 'recommendations_by_device', 'label' => 'Recommendations by category and device'],
            ['key' => 'recommendations_summary', 'label' => 'Recommendations summary by category'],
            ['key' => 'expected_savings', 'label' => 'Expected Savings'],
            ['key' => 'understanding_bills', 'label' => 'Understanding Energy Bills'],
        ];
    @endphp
    <div class="toc-page">
    ...
    <table>
        ...
        <tbody>
            @foreach ($tarrifValuesTable as $tarrif)
                <tr>
                    <td>{{ $formatPdfCell($tarrif->source?->name ?? $tarrif->source_id) }}</td>
                    <td>{{ $formatPdfCell($tarrif->source?->unit ?? '') }}</td>
                    <td>{{ number_format((float) $tarrif->unit_cost, 2) }} NIS</td>
                </tr>
            @endforeach
        </tbody>
    </table>
...
of {{ $report->property->number_of_floors }} floors. with overall area of {{ $report->property->property_area }} meter square, and total of {{ $report->property-> number_of_rooms }} rooms</div>
    ...
    @foreach ($groupedDevices as $categoryId => $devices)
        ...
        <p>{{ $formatPdfCell($descriptionsnData[$categoryId] ?? '') }}</p>
        <table>
            <thead>
                <tr>
                    <th>Description</th>
    ...
    <div>
            @foreach ($recommendationData['recommendations'] ?? [] as $key=>$recommendation)
                <div>
                    <div>{{ $formatPdfCell($key) }}</div>
                    <p>{{ $formatPdfCell($recommendation) }}</p>
                </div>
            @endforeach
    </div>
    ...
    <table>
        ...
        @foreach (is_array($recommendation) ? $recommendation : [] as $key=>$recommendation2)
                <tr>
                    <td>{{ $formatPdfCell($key) }}</td>
                    <td>{{ $formatPdfCell(is_array($recommendation2) ? ($recommendation2['current_energy_use_kWh'] ?? null) : null) }}</td>
                    <td>{{ $formatPdfCell(is_array($recommendation2) ? ($recommendation2['energy_use_after_recommendations_kWh'] ?? null) : null) }}</td>
                    <td>{{ $formatPdfCell(is_array($recommendation2) ? ($recommendation2['saving_kWh'] ?? null) : $recommendation2) }}</td>
                </tr>
            @endforeach
        </table>
    @endforeach
    ...
    <table>
        <tr>
            ...
            @foreach ($recommendationTableDataObj as $key=>$recommendation)
                <tr>
                    <td>{{ $formatPdfCell($key) }}</td>
                    <td>{{ $formatPdfCell(is_array($recommendation) ? ($recommendation['current_energy_use_kWh'] ?? null) : null) }}</td>
                    <td>{{ $formatPdfCell(is_array($recommendation) ? ($recommendation['energy_use_after_recommendations_kWh'] ?? null) : null) }}</td>
                    <td>{{ $formatPdfCell(is_array($recommendation) ? ($recommendation['saving_kWh'] ?? null) : $recommendation) }}</td>
                </tr>
            @endforeach
        </table>
